...
begin
    ...
    IF (TG_OP = 'UPDATE' AND NEW.date = OLD.date AND NEW.start_time = OLD.start_time
        RETURN NEW;

    -- Check if it is a valid operation
    ELSE IF (TG_OP NOT IN ('INSERT', 'UPDATE')) THEN
        RAISE EXCEPTION "Trigger is not suppose to be enforces in other methods.";
    END IF;

    -- Find collisions
    SELECT COUNT(*) INTO collisions_same_course_offering 
    FROM Sessions s
